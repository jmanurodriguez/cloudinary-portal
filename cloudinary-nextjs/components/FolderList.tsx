  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center py-12">
        ...
        <button onClick={loadFolders} className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors flex items-center gap-2">
  ...
  if (folders.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12">
        ...
        <p className="text-sm text-gray-500 text-center">Crea una carpeta para comenzar</p>
        <button onClick={loadFolders} className="mt-4 px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors flex items-center gap-2">
          <RefreshCw className="w-4 h-4" />
          Actualizar
        </button>
      </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h3 className="text-xl font-semibold text-gray-800">Carpetas Disponibles ({folders.length})</h3>
        <button onClick={loadFolders} className="p-2 text-gray-500 hover:text-blue-500 transition-colors rounded-lg hover:bg-blue-50" title="Actualizar carpetas">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {folders.map((folder) => (
          <FolderCard key={folder.path} folder={folder} onSelect={onFolderSelect} onDelete={canDelete ? handleDeleteFolder : undefined} isAdmin={canDelete} isDeleting={deletingFolders.has(folder.name)} />
        ))}
      </div>
    </div>
  );
